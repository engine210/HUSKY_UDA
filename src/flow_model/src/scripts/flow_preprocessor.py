...
def callback(ros_data):
    ...
    # img = cv2.resize(img, (336, 184))
    # img = cv2.resize(img, (336, 188))

    # zed camera size
    # img = cv2.resize(img, (672, 376))

    # unity size
    # img = cv2.resize(img, (180, 84))
    img = cv2.resize(img, (360, 168))
    # img = cv2.resize(img, (360, 168))
    # img = cv2.resize(img, (720, 336))
...
def main(args):
    ...
    # model.load_state_dict(torch.load("./RAFT/models/raft-kitti.pth"))
    ...
    pub = rospy.Publisher("/processed/flow_all_raw/compressed",
